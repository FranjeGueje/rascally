...
def create_lnk(*args):
	logger.debug('[create_lnk]Creating a temporally script to create the link')
	
	bashfile=''.join(random.choice(string.ascii_uppercase + string.digits) for _ in range(10))
	bashfile='/tmp/'+bashfile+'.sh'
 
	logger.debug(f'[create_lnk]Created a temporally script to create the link: {bashfile}')
	f = open(bashfile, 'w')
	s = """#!/bin/bash
...
"""
	...
	logger.debug('[create_lnk]Finnished')