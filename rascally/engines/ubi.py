...
class EngineUbisoft(LauncherEngine):
    ...
    def search_games(self, search_path=os.environ.get("HOME") + "/.local/share/Steam", excep={}) -> Tuple[BasicGame]:
        ...
        for carpeta in carpetas:
            ...
            path_reg = os.path.join(path, carpeta + "/pfx/system.reg")
            exe_engine = 'UbisoftConnect.exe'.lower()

            if os.path.isfile(path_reg):
                try:
                    with open(path_reg, 'r') as f:
                        for linea in f:
                            if exe_engine in linea.lower():
                                logger.info(
                                ...
                                for i in range(1, len(juegos_bruto)):
                                    ...
                                    install_dir = juegos_bruto[i].split("\"")[
                                        3]
                                    title = os.path.basename(install_dir[1:-1])
                                    logger.info(
                                        f'[EngineUbisoft]Found the game {title} with Exe {exe_completed}')
                                    g = BasicGame(
                                        AppName=title,
                                        Exe=exe_completed,
                                        CompatData=path + carpeta
                                    )
                                    juegos.append(g)
                                break
                    continue
